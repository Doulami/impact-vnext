...
function CheckoutPageContent() {
  ...
  const handlePaymentSubmit = async (methodCode?: string) => {
    ...
    if (!paymentMethod) {
      setError(t('errors.selectPaymentMethod'));
      return;
    }
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between">
              {[
                { num: 1, label: t('steps.shipping') },
                { num: 2, label: t('steps.delivery') },
                { num: 3, label: t('steps.payment') },
                { num: 4, label: t('steps.confirmation') }
              ].map((step, idx) => (
                <div key={step.num} className="flex items-center">
                  ...
                  <span className="ms-2 text-sm font-medium">{step.label}</span>
          ...
          {currentStep === 1 && (
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <form onSubmit={handleShippingSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('form.streetAddress')} *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('form.apartment')}</label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {countriesLoading ? (
                      ...
                      <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-gray-400 me-2"></div>
                        <span className="text-gray-500 text-sm">{t('loading.countries')}</span>
                      </div>
                    ) : (
                      <select
                        required
          ...
          {currentStep === 2 && (
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h2 className="text-xl font-bold mb-6">{t('selectShippingMethod')}</h2>
              
              {shippingMethodsLoading ? (
                <div className="flex items-center justify-center py-8">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[var(--brand-primary)]"></div>
                  <span className="ms-3 text-gray-600">{t('loading.shippingMethods')}</span>
                </div>
              ) : shippingMethodsError ? (
                <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                  <p className="text-red-800">{t('errors.loadingShippingError')}</p>
                  <p className="text-sm text-red-600 mt-2">{shippingMethodsError.message}</p>
                </div>
              ) : shippingMethods.length === 0 ? (
                <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <p className="text-yellow-800">{t('errors.noShippingMethods')}</p>
                  <p className="text-sm text-yellow-700 mt-2">{t('errors.contactSupport')}</p>
                  <Button
                  ...
                  <div className="space-y-4">
                    {shippingMethods.map((method: any) => (
                      ...
                      >
                        ...
                        <span className="ms-4 text-gray-600">${(method.priceWithTax / 100).toFixed(2)}</span>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                      ...
                      {item.isBundle ? (
                        ...
                        <div className="flex gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center justify-between">
                              <span className="text-xs text-gray-500">{tCart('qty')}: {item.quantity}</span>
                ...
                <div className="border-t pt-6">
                  <h3 className="text-lg font-semibold mb-3">{t('coupon.title')}</h3>
                  <CouponCodeInput
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                <h2 className="text-xl font-bold mb-6">{t('selectPaymentMethod')}</h2>
                ...
                <div className="space-y-4 mb-6">
                  {paymentMethodsData?.eligiblePaymentMethods?.length ? (
                    ...
                    <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-center">
                      <p className="text-yellow-800">{t('errors.noPaymentMethods')}</p>
                      <p className="text-sm text-yellow-700 mt-1">{t('errors.contactSupportPayment')}</p>
                    </div>
                  )}
                </div>
          ...
          {currentStep === 4 && (
            <div className="bg-white rounded-lg shadow-sm p-6 text-center">
              ...
              <p className="text-gray-600 mb-4">{t('confirmation.orderNumber')} <strong>{orderCode}</strong></p>
              <p className="text-gray-600 mb-6">{t('confirmation.emailSent', { email: customer?.emailAddress || '' })}</p>
