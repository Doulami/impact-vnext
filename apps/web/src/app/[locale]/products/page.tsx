...
import { filterSearchResultsByLanguage } from '@/lib/utils/productLanguageValidation';
...
function ProductsPageContent() {
  ...
  // Filter products by language - only show products with name/description in current language
  const languageFilteredProducts = filterSearchResultsByLanguage(products, locale);
  ...
  const productCards = languageFilteredProducts.map(p => toProductCardData(p, bundleFacetValueIds));
          ...
          <div className="flex items-start justify-between">
            <div className="flex-shrink-0">
              ...
              <p className="text-gray-600">
                {t('productsAvailable', { count: totalItems })}
              </p>
            </div>
              ...
              <div className="min-h-[60px] flex items-center justify-end">
                {activeFilters.length > 0 ? (
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    <span className="text-sm font-medium whitespace-nowrap">{t('activeFilters')}</span>
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <aside className="lg:col-span-1">
            <div className="bg-white p-6 rounded-lg sticky top-24">
              ...
              {/* Stock Status */}
              <div className="mb-6">
                <h3 className="font-semibold mb-3 uppercase">{t('availability')}</h3>
          ...
          <main className="lg:col-span-3">
            ...
            <div className="bg-white p-4 rounded-lg mb-6 flex items-center justify-between">
              <div className="text-sm text-gray-600">
                {t('showing', { current: productCards.length, total: totalItems })}
              </div>
            ...
            {error && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
                <p className="text-red-600 font-medium mb-2">{t('unableToLoad')}</p>
            ...
            {/* Empty Language State - When products exist but none for current language */}
            {!loading && !error && products.length > 0 && productCards.length === 0 && (
              <EmptyLanguageState />
            )}
                      ...
                      <div className="p-3">
                        ...
                        <div className="space-y-2">
                          ...
                          {product.inStock ? (
                            ...
                            >
                              {product.isBundle ? t('addToCart').toUpperCase() : product.priceRange ? t('chooseOptions').toUpperCase() : t('addToCart').toUpperCase()}
                            </button>
                          ) : (
                            <button
                              disabled
